...
class REWARD(Enum):
    ...
    VALID_DEFENSE = 50      # When a defense is detected as positive change in the environment
...
class SecurityEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        is_improved = np.count_nonzero(observation == 3) > np.count_nonzero(self._last_observation == 3)
        ...
        # In max steps, do not give advantage otherwise the last action is set as valid and it is not
        if self._steps >= self.MAX_STEPS:
            reward += 0
            truncated = True
        elif not is_damaged and np.count_nonzero(observation == 0) != 3:
            reward += 5
            terminated = True
        return self._result(action, observation, reward, terminated, truncated, '')

        # if self._steps < self.MAX_STEPS:
        #     return self._result(action, observation, reward, False, False, '')
        # else:
        #     if is_damaged:
        #         reward -= 10
        #     else:
        #         reward += 5
        #     return self._result(action, observation, reward, True, False, 'Max steps')
    ...
    def step_auto(self, action):
        ...
        # Check if the action is "expensive"
        action_expensive = self.ACTIONS[action].get('name') != self.ACTIONS[0].get('name')
        ...
        # Determine if the state is damaged
        is_damaged = self._is_damaged(observation)

        # Determine if the state has improved
        is_improved = np.count_nonzero(observation == 3) > np.count_nonzero(self._last_observation == 3)

        # Reward for improvement
        if is_improved:
            reward += 10  # Strong reward for improvement

        # Penalty for expensive action without improvement
        if action_expensive and not is_improved:
            reward -= 2

        # Small reward for non-expensive actions
        if not action_expensive:
            reward += 1

        # End of episode rewards or penalties
        if self._steps >= self.MAX_STEPS:
            if is_damaged:
                reward -= 20  # Strong penalty for ending in a damaged state
            else:
                reward += 10  # Moderate reward for ending in a non-damaged state
            return self._result(action, observation, reward, True, False, 'Max steps')
